...
# the optimal solution with sliding window approach 
def optimal_solution(arr, k):
    count = 0
    last_seen = {ch: -1 for ch in k}
    for i in range(len(arr)):
        last_seen[arr[i]] = i

        if last_seen[k[0]] != -1 and last_seen[k[1]] != -1 and last_seen[k[2]] != -1:
            count += (1+min(last_seen[k[0]], last_seen[k[1]], last_seen[k[2]]))
    return count
